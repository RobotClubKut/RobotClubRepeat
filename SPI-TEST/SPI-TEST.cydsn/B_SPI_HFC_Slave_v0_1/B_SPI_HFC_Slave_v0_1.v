...
module B_SPI_HFC_Slave_v0_1(
	...
	output  wire tde,           /* tx data effective                */
	...
	wire dp_clock;
	generate
	if ((ModeCPHA == 0 && ModePOL == 0) || (ModeCPHA == 1 && ModePOL == 1)) begin
	...
	wire load          = (ModeCPHA == 0) ? dpcounter_one : dpcounter_zero;
	...
	wire mosi_buf_overrun = (load & dpMOSI_fifo_full);
	...
	wire mosi_to_dp = (load == 1'b1) ? mosi : mosi_tmp;
	wire load_rx =  rde_reg & load & inv_ss;

	reg tde_reg;
	always @(posedge dp_clock) begin
		if (load) begin
			tde_reg <= dpMISO_fifo_empty;
		end
		else begin
			tde_reg <= tde_reg;
		end
	...
	assign tde = dpMISO_fifo_empty & tde_reg;
	...
	if (NumberOfDataBits <= SR8) begin: sR8
		...
		Dp(
			/*  input           */ .clk(dp_clock),
			/*  input           */ .reset(reset),
			/*  input   [02:00] */ .cs_addr({inv_ss, 1'b0, load}),
